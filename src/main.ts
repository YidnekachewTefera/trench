...
async function bootstrap(nodeNumber: number) {
  ...
  if (process.env.NODE_ENV === 'development') {
    ...
    try {
      ...
      logger.log('trying to write to ./swagger-spec.json')
      fs.writeFileSync('./swagger-spec.json', JSON.stringify(document, null, 2))
      logger.log('Swagger spec written to ./swagger-spec.json')
    } catch (error) {
      // If permission denied, try writing to /tmp directory
      if (error.code === 'EACCES') {
        try {
          const tmpPath = '/tmp/swagger-spec.json'
          fs.writeFileSync(tmpPath, JSON.stringify(document, null, 2))
          logger.log(`Swagger spec written to ${tmpPath} (permission denied for current directory)`)
        } catch (tmpError) {
          logger.warn('Could not write swagger spec file:', tmpError.message)
        }
      } else {
        logger.warn('Could not write swagger spec file:', error.message)
      }
    }
